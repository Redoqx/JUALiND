...
func NewMux(userRepo *repository.UserRepository, productRepo *repository.ProductRepository, orderRepo *repository.OrderRepository) *http.ServeMux {
	...
	m.Handle("/api/v1/login", middleware.AllowOrigin(middleware.Method("POST", router.LoginUser(userRepo))))
	...
	m.Handle("/api/v1/order/csv", middleware.AllowOrigin(middleware.Method("GET", middleware.AuthMiddleware(middleware.Role("penjual", router.GenerateFileByUser(orderRepo))))))
	return m
}
