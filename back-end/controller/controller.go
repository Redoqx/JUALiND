...
func NewMux(userRepo *repository.UserRepository, productRepo *repository.ProductRepository) *http.ServeMux {
	...
	m.Handle("/api/v1/product/create", middleware.Method("POST", middleware.AuthMiddleware(middleware.Role("penjual", router.CreateProduct(productRepo)))))
	m.Handle("/api/v1/product/update", middleware.Method("PUT", middleware.AuthMiddleware(middleware.Role("penjual", router.UpdateProduct(productRepo)))))
